...
class TaskController extends Controller
{
    ...
    //获取叮叮需要的数据
    public function actionSendDingding($taskId)
    {
        ...
        $outputs = [];
        if (is_array($params['dingding'])) {

            foreach ($params['dingding'] as $url) {
                $outputs[] = $this->_sendDingding($url, $params);
            }
        }

        static::renderJson(['p' => $params, 'info' => $outputs]);

    }

    private function _sendDingding($url, $params = [])
    {
